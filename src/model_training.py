...
class ModelTraining:
    def __init__(self):
        self.data_path = PROCESSED_DIR
        self.experiment = comet_ml.Experiment(
            api_key = "RWBNrcDyACSrrNTsLEePoNqLE",
            project_name = "projanimerecommender",
            workspace = 'pranabmir'
        )
    ...
    def train_model(self):
        try:
            ...
            users_len = len(joblib.load(USER2USERENCODED))
            anime_len = len(joblib.load(ANIME2ANIMEENCODED))
            print(users_len)
            print(anime_len)

            base_model = BaseModel(config_path=CONFIG_PATH)

            model=  base_model.RecommenderNet(n_users=users_len,n_anime=anime_len)
            start_lr = 0.00001
            min_lr = 0.0001
            max_lr = 0.00005
            batch_size = 10000
            ...
            logger.error(str(e))
            raise CustomException("Error while model training process",e)
